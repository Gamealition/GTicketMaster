...
public class CommandReopen extends Command {
	...
	protected void run(CommandSender sender, String[] args) {
		...
		if(args.length != 2) {
			Lang.sendErrorMessage(sender, Lang.REOPEN_COMMAND_USAGE);
			return;
		}
		...
		if(status != TicketStatus.CLOSED) {
			Lang.sendErrorMessage(sender, Lang.REOPEN_COMMAND_NOT_CLOSED);
			return;
		}
		
		// Change status
		boolean success = Controller.setTicketStatus(id, TicketStatus.PENDING);
		
		// Success
		if(success) {
			// Add log to ticket as a comment
			TicketComment log = new TicketComment(id, sender.getName(), TicketLogBuilder.reopened());
			Controller.pushTicketComment(log);

			// Message command sender
			String msg = LangMacro.replaceId(Lang.REOPEN_COMMAND_SUCCESS, id);
			Lang.sendMessage(sender, msg);
			
			// Message ticket submitter
			String actionMessage = TicketActionBuilder.reopened(sender.getName(), id);
		...
		// Failure
			...
			String msg = LangMacro.replaceId(Lang.REOPEN_COMMAND_FAILURE, id);
